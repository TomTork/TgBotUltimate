...
import (
	...
	"TgBotUltimate/server/routes/external/helper"
	helper2 "TgBotUltimate/server/routes/helper"
...
func Feed(ctx context.Context) string {
	...
	if err := helper2.Get(ctx, fmt.Sprintf("%s/get_projects", os.Getenv("URL_1C")), nil, &__projects); err != nil {
		return err.Error()
	}
	for _, __project := range __projects {
		var data Sync1C.Data
		if err := helper2.Get(ctx, fmt.Sprintf("%s/get_all?project_id=%s", os.Getenv("URL_1C"), __project.Uid), nil, &data); err != nil {
			return err.Error()
		}
		for _, project := range data.Projects {
			_project := Sync1C.TypeProject{ProjectId: project.ProjectId, ProjectName: project.ProjectName}
			if err := data2.CreateProject(ctx, db, helper.ConvertProjectToType1C(_project)); err != nil {
				return err.Error()
			} else if err := data2.UpdateProject(ctx, db, helper.ConvertProjectToType1C(_project)); err != nil {
				return err.Error()
			}
			for _, house := range project.Houses {
				for _, building := range house.Buildings {
					...
					if err := data2.CreateBuilding(ctx, db, helper.ConvertBuildingToType1C(_building)); err != nil {
						return err.Error()
					} else if err := data2.UpdateBuilding(ctx, db, helper.ConvertBuildingToType1C(_building)); err != nil {
						return err.Error()
					}
					for _, apartment := range building.Apartments {
						...
						if err := data2.CreateFlat(ctx, db, helper.ConvertApartmentToType1C(_flat)); err != nil {
							return err.Error()
						} else if err := data2.UpdateFlat(ctx, db, helper.ConvertApartmentToType1C(_flat)); err != nil {
							return err.Error()
						}
					}
				}
